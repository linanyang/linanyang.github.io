...
var Countdown = () => {
    ...
    // 截至时间戳 and 星期对象
    let Countdown = new Date('2024-06-07 09:00:00').getTime() / 1000,
    ...
    function time() {
        ...
        if (second < 0) {
            ...
            // 大于一天倒计时
            if (second > 86400) {
                let d = nol(parseInt(second / 86400));
                second %= 86400
                ...
                second %= 60;
                let s = nol(second);
                document.querySelector('#Countdown .Countdown-time').innerHTML = `<span class="time">${d}天${h}时<br>${m}分${s}秒</span></span>`;
                // 计时
